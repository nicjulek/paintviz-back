...
export class PecaController {
    ...
    async aplicarCorNaPeca(req: Request, res: Response) {
        try {
            ...
            const { id_cor, svg_carroceria } = req.body;

            if (!id_cor || !svg_carroceria) {
                return res.status(400).json({
                    error: 'id_cor e svg_carroceria são obrigatórios'
                });
            }
            ...
            const cor = await this.pecaRepository.findCorById(id_cor);
            if (!cor) {
                return res.status(404).json({ error: 'Cor não encontrada' });
            }

            const svgAtualizado = aplicarCorNoSvg(svg_carroceria, peca.id_svg, cor.cod_cor);
